...
final class ManticoreSearchEngine implements SearchEngineInterface
{
     ...
     *
     * Метка обязана пережить три чужих разбора подряд, и каждый из них отбирает символы:
     *
     *  * `Connection::quoteSql()` — Yii раскрывает `[[имя]]` в имя столбца в кавычках, а `{{имя}}`
     *    в имя таблицы. Это происходит внутри `Command`, то есть с ЛЮБЫМ запросом; метка вида
     *    `[[bescms-mark]]` доезжала до демона как `` `bescms-mark` `` и вылезала в выдачу;
     *  * язык запросов Manticore — служебными являются скобки, `|`, `-`, `~`, `^`, `@`, `/`;
     *  * `Html::encode()` — метки заменяются уже ПОСЛЕ экранирования, поэтому `<`, `>`, `&`,
     *    кавычки в метке недопустимы: до замены они успеют превратиться в сущности.
     *
     * Символы приватной области Unicode не задевает ни одно из трёх правил, и в тексте сайта они
     * не встречаются.
     */
    private const string MARK_OPEN = "\u{E000}";

    private const string MARK_CLOSE = "\u{E001}";
